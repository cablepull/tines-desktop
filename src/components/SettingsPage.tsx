...
export default function SettingsPage({ tenant, apiKey }: SettingsPageProps) {
  ...
  const [debugLookbackHours, setDebugLookbackHours] = useState(getStoredLookbackHours);
  ...
  useEffect(() => {
    window.localStorage.setItem('tinesDesktop.debugLookbackHours', String(debugLookbackHours));
  }, [debugLookbackHours]);
    ...
    <div style={{ flex: 1, padding: '2rem 3rem', overflowY: 'auto' }}>
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={sectionStyle}>
            <div style={labelStyle}>DEBUGGING</div>
            <div style={cardStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
                <div>
                  <div style={{ fontWeight: 600, color: 'white' }}>Default All-Runs Debug Window</div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.25rem' }}>
                    When no execution run is selected, debug tallies aggregate all runs within this lookback window.
                  </div>
                </div>
                <select
                  value={String(debugLookbackHours)}
                  onChange={(e) => setDebugLookbackHours(Number(e.target.value))}
                  className="btn-glass"
                  style={{ minWidth: '160px', padding: '0.5rem', background: 'rgba(0,0,0,0.2)', border: '1px solid var(--glass-border)', color: 'white', borderRadius: '8px' }}
                >
          ...
          <div style={{ ...sectionStyle, marginTop: '4rem', padding: '2rem', borderRadius: '12px', background: 'rgba(59, 130, 246, 0.05)', border: '1px solid rgba(59, 130, 246, 0.15)' }}>
            <div style={{ ...labelStyle, color: '#60a5fa' }}>EDITOR SURFACE</div>
            <div style={{ fontWeight: 600, color: 'white' }}>Mutation controls moved to `Editor`</div>
            <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.25rem' }}>
              Story creation, editable canvases, and chaos test-bed tools are now isolated in the `Editor` section so Settings remains configuration-only.
            </div>
          </div>
        </>
      )}
